...
##########################
  ...
  @reachability_status_subscriptions_01_operation_to_retrieve_list_of_subscriptions_2legs
  Scenario: Get a list of subscriptions for a 2-legged access token
    Given an API consumer that has created at least one reachability status subscription
  ...
  @reachability_status_subscriptions_02_operation_to_retrieve_list_of_subscriptions_3legs
  Scenario: Get a list of subscriptions for a 3-legged access token
    Given an API consumer that has created at least one reachability status subscription for a given device
  ...
  @reachability_status_subscriptions_04_pagination_default_values
  Scenario: Subscription list pagination with default values for page and perPage
    Given an API consumer who has created more than 20 reachability status subscriptions
    When the request "retrieveDeviceReachabilityStatusSubscriptionList" is sent without setting query parameters "page" and "perPage"
    Then the response code is 200
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has the same value as the request header "x-correlator"
  ...
  @reachability_status_subscriptions_05_pagination_custom_values
  Scenario: Subscription list pagination with custom values for page and perPage
    Given an API consumer who has created more than 40 reachability status subscriptions
    When the request "retrieveDeviceReachabilityStatusSubscriptionList" is sent with query parameters "page" set to 1 and "perPage" set to 20
    Then the response code is 200
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has the same value as the request header "x-correlator"
    And the response header "Link" contains a link to the next page with rel="next"
  ...
  @reachability_status_subscriptions_06_pagination_middle_page
  Scenario: Subscription list pagination fetching a middle page of the list
    Given an API consumer who has created more than 40 reachability status subscriptions
    When the request "retrieveDeviceReachabilityStatusSubscriptionList" is sent with query parameters "page" set to 2 and "perPage" set to 20
    Then the response code is 200
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has the same value as the request header "x-correlator"
  ...
  @reachability_status_subscriptions_07_pagination_last_page
  Scenario: Subscription list pagination fetching the last page of the list
    Given an API consumer who has created more than 40 and less than 60 reachability status subscriptions
    When the request "retrieveDeviceReachabilityStatusSubscriptionList" is sent with query parameters "page" set to 3 and "perPage" set to 20
    Then the response code is 200
    And the response header "Content-Type" is "application/json"
    And the response header "x-correlator" has the same value as the request header "x-correlator"
...
##################
  ...
  @reachability_status_subscriptions_retrieve_list_400.01_pagination_invalid_page_parameter
  Scenario: Subscription list pagination with invalid value for page parameter
    Given an API consumer who has created more than 20 reachability status subscriptions
    When the request "retrieveDeviceReachabilityStatusSubscriptionList" is sent with query parameter "page" set to any value less than 1 and "perPage" set to 20
  ...
  @reachability_status_subscriptions_retrieve_list_400.02_pagination_invalid_perPage_parameter
  Scenario: Subscription list pagination with invalid value for perPage parameter
    Given an API consumer who has created more than 20 reachability status subscriptions
    When the request "retrieveDeviceReachabilityStatusSubscriptionList" is sent with query parameter "page" set to 1 and "perPage" set to any value outside the range [1-100]
    Then the response status code is 400
    And the response header "Content-Type" is "application/json"
    And the response property "$.status" is 400
    And the response property "$.code" is "INVALID_ARGUMENT"
